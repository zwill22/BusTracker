...
struct VehicleDetailMap: View {
    ...
    func getPlace() -> VehiclePlace? {
        if let location = vehicle?.details.location {
            return VehiclePlace(location: location)
        }
    ...
    func updatePosition(
        latitudeScale: CGFloat = 2.0,
        longitudeScale: CGFloat = 1.2
    ) -> MapCameraPosition {
        let place = getPlace()
        ...
        if let destinationLocation = getDestinationLocation() {
            if let location = place?.location {
                ...
                let sumLongitude =
                    location.longitude + destinationLocation.longitude
                let centre = CLLocationCoordinate2D(
                    latitude: sumLatitude / 2,
                    longitude: sumLongitude / 2
                )

                let deltaLatitude = max(
                    abs(location.latitude - destinationLocation.latitude),
                    0.001
                )
                let deltaLongitude = max(
                    abs(location.longitude - destinationLocation.longitude),
                    0.001
                )
    ...
    var body: some View {
        let place = getPlace()
        let tintColour =
            vehicle?.vehicleOperator?.primaryColour ?? .primary.opacity(0.80)
