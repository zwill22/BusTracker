...
struct VehicleDetail: View {
    ...
    var body: some View {
        ...
        NavigationStack {
            VStack(alignment: .leading) {
                ...
                HStack {
                    ...
                    VStack(alignment: .leading) {
                        if let destination = vehicle?.destination {
                            NavigationLink(
                                destination: StopDetailView(stop: destination)
                            ) {
                                Text(destination.name).lineLimit(1).font(.title)
                            }.foregroundStyle(.primary)
                        } else {
                            Text(vehicle?.details.destination ?? "").lineLimit(
                                1,
                                reservesSpace: true
                            ).font(.title)
                        }
                        ...
                        if let origin = vehicle?.origin {
                            NavigationLink(
                                destination: StopDetailView(stop: origin)
                            ) {
                                Text("Origin: \(origin.name)").lineLimit(
                                    1,
                                    reservesSpace: true
                                ).font(.headline)
                            }.foregroundStyle(.primary)
                        ...
                        if let transportOperator = vehicle?.vehicleOperator {
                            ...
                            Text(
                                "Operator: \(vehicle?.details.operatorCode ?? "")"
                            )
                        ...
                        Menu {
                            ...
                            VStack(alignment: .leading) {
                                Text(
                                    "Latitude: \(location?.latitude.formatted(.number.precision(.fractionLength(3))) ?? "")"
                                )
                                Text(
                                    "Longitude: \(location?.longitude.formatted(.number.precision(.fractionLength(3))) ?? "")"
                                )
                            }
                        }.foregroundStyle(.primary)
                    }
                ...
                .padding(
                    EdgeInsets(top: 0, leading: 20, bottom: 10, trailing: 10)
                )
            }
        }
    ...
    func updateVehicle() async {
        ...
        do {
            if let index = offset {
                try await vehicleProvider.updateVehicle(atOffset: index)
    ...
    init(offset: Int, vehicleProvider: VehicleProvider) {
        ...
        if offset < vehicleProvider.vehicles.count {
